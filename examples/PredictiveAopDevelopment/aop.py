...
def show_intro():
    ...
    text.append("This tool downloads and processes CTD data on chemicals, diseases,\n", style="bold green")
    text.append("phenotypes, and genes.\n", style="bold green")
    panel = Panel(text, title="Welcome", title_align="left", border_style="bright_yellow")
    console.print(panel)
...
def get_valid_file_name():
    ...
    while True:
        ...
        if input_file in files:
            ...
            console.print(f"[bold red]File '{input_file}' not found. Please try again.[/bold red]")
...
def ctd_download(input_type, input_terms, report, format, file_base_name, ontology_association=None):
    """
    ...
    for i, start in enumerate(range(0, len(input_terms), MAX_TERMS)):
        ...
        if response.status_code == 200:
            ...
            logging.error(f"Failed to download chunk {i + 1}. Status code: {response.status_code}")
            break
...
def find_common_genes(row):
    ...
    """
    chemical_genes = str(row["ChemicalGeneSymbol"]).split(",") if pd.notna(row["ChemicalGeneSymbol"]) else []
    disease_genes = str(row["DiseaseGeneSymbol"]).split(",") if pd.notna(row["DiseaseGeneSymbol"]) else []
...
def run_ctd_pipeline(chemical_file):
    ...
    logging.info(f"Unique DiseaseID Count: {len(df_raw_chemical_disease['DiseaseID'].unique())}")
    logging.info(f"Unique CasRN Count: {len(df_raw_chemical_disease['CasRN'].unique())}")
    ...
    aggregated_chemical_disease = aggregate_dataframe(
        df_chemical_disease, ["# Input", "ChemicalName", "ChemicalID", "CasRN"], "DiseaseID"
    )
    aggregated_chemical_disease.to_csv("aggregated_chemical_disease.csv", index=False)
    logging.info(f"Shape of aggregated_chemical_disease.csv: {aggregated_chemical_disease.shape}")
    ...
    aggregated_chemical_phenotype = aggregate_dataframe(
        df_chemical_phenotype, ["# Input", "ChemicalName", "ChemicalID", "CasRN"], "PhenotypeID"
    )
    ...
    aggregated_chemical_gene = aggregate_dataframe(
        df_chemical_gene, ["# Input", "ChemicalName", "ChemicalId", "CasRN"], "GeneSymbol"
    )
    ...
    logging.info(f"Columns in aggregated_chemical_gene: {aggregated_chemical_gene.columns}")
    ...
    merged_cd_pheno_gene.rename(columns={"GeneSymbol": "ChemicalGeneSymbol"}, inplace=True)
    ...
    merged_final["common_gene_chemical_disease"] = merged_final.apply(find_common_genes, axis=1)
    exportcsv(merged_final, "cd_common_gene")

    # Step 11: Extract unique common genes and download gene ontology data.
    extract_unique_and_save(pd.read_csv("cd_common_gene.csv"), "common_gene_chemical_disease", "cd_common_gene_list.txt")
    unique_genes = set()
    ...
    aggregated_go = aggregate_dataframe(df_raw_go, ["# Input", "GeneSymbol"], "GoTermID")
    ...
    gene_go_dict = df_aggregated_go.groupby("GeneSymbol")["GoTermID"].apply(list).to_dict()
    ...
    for index, row in df_cd_common.iterrows():
        if pd.notna(row["common_gene_chemical_disease"]):
            ...
            df_cd_common.at[index, "disease_gene_ontology"] = ", ".join(go_terms) if go_terms else pd.NA
    ...
    for index, row in final_df.iterrows():
        phenotype_ids = str(row["PhenotypeID"]).split(",") if pd.notna(row["PhenotypeID"]) else []
        disease_ontologies = str(row["disease_gene_ontology"]).split(",") if pd.notna(row["disease_gene_ontology"]) else []
...
def predictive_aop():
    ...
    try:
        ...
        console.print("[bold green]CTD data preparation pipeline completed successfully.[/bold green]")
